...
const server = http.createServer((req, res) => {
  ...
  if (req.url === "/") {
    ...
    res.write(
      '<nav style="background-color:#f8f8f8; padding:10px; font-family:Arial, sans-serif;">'
    );
    res.write(
      '<a href="/" style="margin-right:20px; text-decoration:none; color:#333;"><b>BazarIndian</b></a>'
    );
    res.write(
      '<a href="/men" style="margin-right:20px; text-decoration:none; color:#333;">Men</a>'
    );
    res.write(
      '<a href="/women" style="margin-right:20px; text-decoration:none; color:#333;">Women</a>'
    );
    res.write(
      '<a href="/kids" style="margin-right:20px; text-decoration:none; color:#333;">Kids</a>'
    );
    res.write(
    ...
    res.write("<style>");
    res.write(
      "body { font-family: Arial, sans-serif; background-color: #ffccdd; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }"
    );
    res.write(
      ".form-container { background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); width: 380px; }"
    );
    res.write(
      ".form-container h1 { text-align: center; margin-bottom: 20px; color: #333; }"
    );
    res.write(
      ".form-container label { display: block; margin-bottom: 8px; font-weight: bold; color: #555; }"
    );
    res.write(
      ".form-container input, .form-container textarea, .form-container select { width: 100%; padding: 10px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 8px; }"
    );
    res.write(
      ".form-container button { width: 100%; padding: 12px; background: #ff6699; border: none; border-radius: 8px; color: white; font-size: 16px; cursor: pointer; }"
    );
    ...
    res.write(
      '<label for="title">Update Title (e.g., "Summer Sale 2025")</label>'
    );
    res.write(
      '<input type="text" id="title" name="title" placeholder="Enter the update headline" required>'
    );

    res.write('<label for="details">Details</label>');
    res.write(
      '<textarea id="details" name="details" rows="4" placeholder="Enter full details of the update" required></textarea>'
    );
    ...
    req.on("end", () => {
      ...
      fs.writeFileSync("data.json", JSON.stringify(bodyObject));
    });
